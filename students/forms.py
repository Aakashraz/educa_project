...
class CourseEnrollForm(forms.Form):
    course = forms.ModelChoiceField(
        queryset=Course.objects.none(),     # <- Why empty? - This prevents Django from querying
        # the database when the 'form class' is defined (at module import time).
        widget=forms.HiddenInput            # <- Why hidden? - Because the user doesn't choose the
        # course -- it's passed from the view!
    )
